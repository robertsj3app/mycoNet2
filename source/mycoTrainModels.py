...
def trainModel(whichMold, length_scale=[1.0, 1.0, 1.0], n_restarts_optimizer=2000):
    ...
    if dataChangedSinceLastTrain(whichMold):
        ...
        dataset = []
        query = f"SELECT incubation_days, seed_days, plate_days, yield_per_liter FROM `mold_lots` WHERE mold_id = {whichMold} AND incubation_days > 0 AND seed_days > 0 AND plate_days > 0 AND discarded = 0 and facility != \"Willow Street\" and facility != \"Unknown\" and plug_type != \"Unknown\" ORDER BY `mold_lots`.`mfg_date` ASC;" 
        rawData = mH.SQLQuery(query)
        for d in rawData:
            dataset.append([d["incubation_days"], d["seed_days"], d["plate_days"], d["yield_per_liter"]])
    
        dataset.append([0,0,0,0]) # Define expected behavior for area of curve outside of spec.
...
def trainAllModels(length_scale=[1.0, 1.0, 1.0], n_restarts_optimizer=1000):
    ...
    for i in mold_ids:
        trainModel(i, length_scale=length_scale, n_restarts_optimizer=n_restarts_optimizer)
    mH.toggleLog(False)
